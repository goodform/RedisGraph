...
extern void GraphEntity_Add_Properties(GraphEntity *e, int prop_count, char **keys, SIValue *values);

/* Retrieves entity's property
 * NOTE: If the key does not exist, we return the special
 * constant value PROPERTY_NOTFOUND. */
extern SIValue* GraphEntity_Get_Property(const GraphEntity *e, const char* key);

/* Release all memory allocated by entity */
extern void FreeGraphEntity(GraphEntity *e);
